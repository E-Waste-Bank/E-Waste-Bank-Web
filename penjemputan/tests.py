...
class PenjemputanUserViewsTest(TestCase):
    def setUp(self):
    ...
    def test_penjemputan_page_routing_admin(self):
        response = self.client_admin.get("/penjemputan/")
        self.assertEqual(response.status_code, 200)

    def test_penjemputan_page_routing_user(self):
        response = self.client.get("/penjemputan/")
        self.assertEqual(response.status_code, 200)

    def test_penjemputan_user_page_html_response_ok(self):
        response = self.client_admin.get("/penjemputan/")
        self.assertContains(response, "Penjemputan")

    def test_penjemputan_user_page_html_response_not_ok(self):
        response = self.client.get("/penjemputan/")
        self.assertContains(response, "Penjemputan")

    def test_penjemputan_admin_get_all_response_ok(self):
        response = self.client_admin.get("/penjemputan/json/")
        self.assertEqual(response.status_code, 200)
    
    def test_penjemputan_client_get_all_response_not_ok(self):
        response = self.client.get("/penjemputan/json/")
        self.assertEqual(response.status_code, 200)